-- 0002_guest_api_full.sql

-- 1. Support Extensions (created in db/bootstrap.sql)
...
-- 3. Complex Geometries (Recursive CTE)
...
GRANT EXECUTE ON FUNCTION guest_api.search_stops_by_name(text, integer) TO ct_guest_role, ct_passenger_role, ct_driver_role, ct_dispatcher_role, ct_controller_role, ct_municipality_role, ct_manager_role, ct_accountant_role, ct_admin_role;
GRANT EXECUTE ON FUNCTION guest_api.plan_route(numeric, numeric, numeric, numeric, numeric, integer, integer) TO ct_guest_role, ct_passenger_role, ct_driver_role, ct_dispatcher_role, ct_controller_role, ct_municipality_role, ct_manager_role, ct_accountant_role, ct_admin_role;
GRANT EXECUTE ON FUNCTION guest_api.plan_route_pgrouting(bigint[], bigint[], integer, integer) TO ct_guest_role, ct_passenger_role, ct_driver_role, ct_dispatcher_role, ct_controller_role, ct_municipality_role, ct_manager_role, ct_accountant_role, ct_admin_role;
GRANT EXECUTE ON FUNCTION guest_api.submit_complaint(text, text, text, text, text, text) TO ct_guest_role, ct_passenger_role;

-- Updated schedule view with vehicle linkage
CREATE OR REPLACE VIEW guest_api.v_schedules AS
SELECT
  route_id,
  work_start_time,
