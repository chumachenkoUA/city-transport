...
-- Driver and Controller operational APIs

-- 1. DRIVER LOGIC

-- Helper: cleanup stale trips
...
BEGIN
    ...
    SET ends_at = COALESCE(
        (SELECT MAX(recorded_at) FROM public.vehicle_gps_logs vgl
         WHERE vgl.vehicle_id = t.vehicle_id AND vgl.recorded_at >= t.starts_at),
...
SELECT t.id, t.starts_at, t.ends_at, t.passenger_count, t.route_id,
       r.number AS route_number, r.direction, r.transport_type_id,
       tt.name AS transport_type, t.vehicle_id, v.fleet_number
FROM public.trips t
