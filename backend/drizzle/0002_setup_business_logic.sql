...
DECLARE v_u_id bigint; v_t_id bigint; v_f_id bigint; BEGIN
    ...
    IF p_fleet IS NOT NULL THEN SELECT id INTO v_t_id FROM public.trips t JOIN public.vehicles v ON v.id = t.vehicle_id WHERE v.fleet_number = p_fleet AND t.ends_at IS NULL ORDER BY t.starts_at DESC LIMIT 1; END IF;
